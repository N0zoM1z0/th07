#!/usr/bin/env python3
"""Call IDA MCP tools after fail-closed TH07 target routing."""
...
async def run(server_name: str, allow_write: bool, schemas: list[str], searches: list[str], calls: list[tuple[str, dict[str, object]]]) -> None:
    async with open_session(server_name) as (session, _initialized):
        ...
        for name, arguments in calls:
            ...
            if name.startswith(WRITE_PREFIXES) and not allow_write:
                raise IdaMcpError("write_not_allowed", f"IDA write tool requires --allow-write: {name}")
            output = await call_json(session, name, arguments)
            print(json.dumps({"tool": name, "content": output}, indent=2, ensure_ascii=False))
