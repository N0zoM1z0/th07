#!/usr/bin/env python3
"""Protocol-native helpers for the registered IDA Pro MCP bridge."""
...
@asynccontextmanager
async def open_session(name: str = DEFAULT_SERVER) -> AsyncIterator[tuple[ClientSession, Any]]:
...
def validate_metadata(metadata: dict[str, Any]) -> None:
    ...
    for field in ("sha256", "md5"):
        ...
        if actual != wanted:
            raise IdaMcpError("target_mismatch", f"IDA {field} mismatch: got {actual!r}, expected {wanted!r}")
    for field, actual_raw, wanted_raw in (
        ...
        if actual != wanted:
            raise IdaMcpError("target_mismatch", f"IDA {field} mismatch: got {actual:#x}, expected {wanted:#x}")
